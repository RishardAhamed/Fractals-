...
public class Fractal {
    static int NumOfThreads = 8;        //Number of Threads to run
        ...
        if ( numOfArgs > 0 ){
            if( args[0].toLowerCase().equals("mandelbrot") ){       //If Input Mandelbrot set
                ...
                if ( numOfArgs == 6 ){                              //Mandelbrot with 5 Arguments
                    ...
                    mandelbrot[1].plotFractal();            //Draw Picture after all the threads are finished 
                }
                ...
                if( numOfArgs == 3 ){                               //Julia with 2 Arguments
                    ...
                    joinThreads( thread );                          //Check whether Threads are finished Process
                    julia[1].plotFractal();                         //Draw Picture after all the threads are finished
                }
    ...
    public static void joinThreads(Thread [] thread) {      //Meathod to Join Threads after they Finish
        for(int i = 0; i <NumOfThreads ; i++){
            try {
                thread[i].join();
